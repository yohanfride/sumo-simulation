...
epsilon = 0.1
qdefault = 0
f =  open('best-path.json')
best_path = json.load(f)
f =  open('percobaan.json')
...
def saveQ(name,df):    
    ...
    with open('node_'+name+'.json', 'w') as outfile:
...
def bestq(qtable, node, currentNode, s_time):
    nextNode = currentNode['next']
    if len(nextNode) == 0:
        return saving(distance,duration,node,s_time,currentNode['lastval'])
...
def getReward(name, total):
    ...
    if(DA>=total):
        ...
        r = 0
    np.savetxt('DA_'+name+'.txt', np.array([DA]))
    return r
...
for episode in range(MaxEpisode):
    ...
    with open('hasil/waktu-'+str(pos+1)+'.json', 'w') as outfile:
        json.dump(vehicle, outfile)

    with open('best-path.json', 'w') as outfile:
        json.dump(best_path, outfile)

    with open('percobaan.json', 'w') as outfile:
